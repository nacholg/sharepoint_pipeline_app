...
function vpIsReviewVoucher(voucher) {
  ...
  const hasPendingName = passengers.some((p) =>
    String(p || "")
      .toUpperCase()
      .includes("NAME PENDING")
  );

  const hasWarnings =
    Array.isArray(voucher.warnings) &&
    voucher.warnings.length > 0;

  const hasErrors =
    Array.isArray(voucher.errors) &&
...
function vpGetSelectedVoucherIds() {
  return Array.from(
    document.querySelectorAll(
      ".voucher-select-checkbox:checked"
    )
  )
    .map((input) =>
      String(input.dataset.voucherId || "")
    )
    .filter(Boolean);
}
...
function vpUpdateSelectionSummary() {
  const summaryEl =
    document.getElementById(
      "voucherPreviewSelectionSummary"
    );

  const hintEl =
    document.getElementById(
      "voucherPreviewRunHint"
    );

  const footerEl =
    document.getElementById(
      "voucherPreviewModalFooterLabel"
    );

  const inlineSummaryEl =
    document.getElementById(
      "voucherPreviewInlineSummary"
    );

  const vouchers =
    vpGetPreviewVouchers();
  ...
  if (inlineSummaryEl) {
    inlineSummaryEl.textContent = text;
  }
  ...
  if (
    ...
    window.runLocalBtn.disabled =
      selectedIds.length === 0;

    window.runLocalBtn.classList.toggle(
      "is-disabled",
      selectedIds.length === 0
    );
  }
  ...
  if (
    window.runSharepointBtn &&
    window.APP_STATE?.data?.voucherPreview
  ) {
    window.runSharepointBtn.disabled =
      selectedIds.length === 0;

    window.runSharepointBtn.classList.toggle(
      "is-disabled",
      selectedIds.length === 0
    );
  }
...
function vpBuildStatusBadge(voucher) {
  ...
  if (hasWarnings || hasReview) {
    return `<span class="badge warning">Revisar</span>`;
  }
...
function vpRenderInlineSummary(data) {
  ...
  const vouchers =
    data?.voucher_candidates || [];

  const selectedIds =
    window.APP_STATE?.data
      ?.selectedVoucherIds || [];

  const okCount = vouchers.filter(
    (v) => !vpIsReviewVoucher(v)
  ).length;

  const reviewCount =
    vouchers.length - okCount;
  ...
  host.innerHTML = `
    <div class="voucher-preview-inline-summary">
      ...
      <div>
        ...
        <strong>
          ${vouchers.length} vouchers detectados
        </strong>

        <p
          id="voucherPreviewInlineSummary"
          class="muted-text"
        >
            ...
            <article class="voucher-preview-card">
              ...
              <div class="voucher-preview-main">
                ...
                <div>
                  ...
                  <span class="voucher-preview-label">
                    Pasajeros
                  </span>
...
async function loadVoucherPreview() {
  const isSharePointMode =
    window.currentMode === "sharepoint";

  const selectedProfile =
    isSharePointMode
      ? sharepointProfileSelect?.value ||
        "default"
  ...
  if (
    ...
    alert(
      "Seleccioná un Excel de SharePoint."
    );
    return;
  }
...
window.bindVoucherPreviewModalEvents =
  bindVoucherPreviewModalEvents;

window.vpSyncSelectedVoucherIds =
  vpSyncSelectedVoucherIds;